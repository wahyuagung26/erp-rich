...
type: API Endpoint
title: List Merk
description: Paginated, filterable, sortable list of product brands. Soft-deleted rows are excluded.
method: GET
path: /merk
status: mock
tags: [merk, read, list]
...
Backs `views/merk/pages/PageMerkTable.vue` (via the `useTableList` composable).
...
{
  "data": [
    { "id": 1, "company_id": 1, "code": "SGT", "name": "Samsung", "deleted_at": null }
  ],
