...
description: CRUD for sales-type master data — maps a sales type to its revenue, COGS, and inventory accounts. Soft-delete, company-scoped.
...
three [Akun Perkiraan](../akun-perkiraan/index.md) accounts — the revenue,
COGS ("HPP"), and inventory accounts it posts to — so a future sales module can
...
[Gudang](../gudang/index.md). Unlike those, the three account references
(`akun_pendapatan_id`, `akun_hpp_id`, `akun_persediaan_id`) **can** be changed on
update — only `code` is locked.
