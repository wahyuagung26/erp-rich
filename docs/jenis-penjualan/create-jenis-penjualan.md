...
description: Add a new sales type. Client supplies `code` + three account ids; server assigns `id` and resolves each account's code/name.
method: POST
...
```json
{ "code": "RC0007", "name": "Bahan Baku", "akun_pendapatan_id": 5, "akun_hpp_id": 6, "akun_persediaan_id": 7 }
```
