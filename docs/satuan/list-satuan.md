...
type: API Endpoint
title: List Satuan
description: Paginated, filterable, sortable list of units of measure. Soft-deleted rows are excluded.
method: GET
path: /satuan
status: mock
tags: [satuan, read, list]
...
Backs `views/satuan/pages/PageSatuanTable.vue` (via the `useTableList` composable).
...
{
  "data": [
    { "id": 1, "company_id": 1, "code": "PCS", "name": "Pieces", "deleted_at": null }
  ],
