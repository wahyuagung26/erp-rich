...
Backs `views/sub-akun/pages/PageSubAkunEdit.vue`. The form shows both the group
picker and the code disabled — this endpoint ignores `group_akun_id` and
...
`group_akun_id`, `code_suffix`, `code`, `id`, and `company_id` are ignored if sent —
...
`200`: `{ "data": { ...SubAkun }, "message": "Sub akun diperbarui" }`
