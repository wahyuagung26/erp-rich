...
const live = () => db.satuan.filter((s) => !s.deleted_at)
...
// Contract: docs/satuan/ — code is user-entered at create, immutable after
// (ignored on update). Soft delete (deleted_at), never hard-removed. Company-scoped
// — see docs/conventions.md#company-scoping.
export function registerSatuan(mock: MockAdapter) {
	mock.onGet('/satuan').reply((config) => {
	...
	mock.onGet(/\/satuan\/\d+$/).reply((config) => {
	...
	mock.onPost('/satuan').reply((config) => {
		const companyId = companyIdOf(config)
		...
		const id = nextId(db.satuan)
		const row: Satuan = { id, company_id: companyId, code: body.code.trim(), name: body.name, deleted_at: null }
		db.satuan.unshift(row)
		return [201, { data: row, message: 'Satuan ditambahkan' }]
	})

	mock.onPut(/\/satuan\/\d+$/).reply((config) => {
		const body = JSON.parse(config.data) as { name: string }
		const idx = db.satuan.findIndex((s) => s.id === idOf(config.url) && !s.deleted_at && s.company_id === companyIdOf(config))
		if (idx === -1) return [404, { message: 'Satuan tidak ditemukan' }]
		// code is immutable — ignored even if the client sends it
		db.satuan[idx] = { ...db.satuan[idx], name: body.name }
		return [200, { data: db.satuan[idx], message: 'Satuan diperbarui' }]
	})

	mock.onDelete(/\/satuan\/\d+$/).reply((config) => {
		const found = db.satuan.find((s) => s.id === idOf(config.url) && !s.deleted_at && s.company_id === companyIdOf(config))
