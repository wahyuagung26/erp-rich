...
export type AccountForm = v.InferOutput<typeof accountSchema>

// Returns { field: message } for the first issue per field, or null if valid.
export function validateAccount(data: unknown): Record<string, string> | null {
	const result = v.safeParse(accountSchema, data)
	if (result.success) return null
	const flat = v.flatten<typeof accountSchema>(result.issues).nested ?? {}
