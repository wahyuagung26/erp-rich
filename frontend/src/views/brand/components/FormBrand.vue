...
import { validateBrand, type BrandForm } from '@/views/brand/schema'

const props = defineProps<{ initial?: Partial<BrandForm>; isEdit?: boolean; submitLabel?: string; loading?: boolean }>()
const emit = defineEmits<{ submit: [BrandForm] }>()
...
function onSubmit() {
	const found = validateBrand(form)
	errors.value = found ?? {}
	...
	<Panel>
		<form class="space-y-8" @submit.prevent="onSubmit">
			...
			<div class="flex gap-2 pt-2">
				...
				<Button variant="secondary" type="button" @click="router.push('/brand')"> <IconX class="h-4 w-4" /> Batal </Button>
			</div>
		</form>
	</Panel>
</template>
