...
import type { SubAccount, TableRow } from '@/utils/types'

const router = useRouter()
const { ask } = useConfirm()
const toast = useToast()

const { columns, pagination, loading, fetchList, handleSort, pageTo, applyFilters } = useTableList<SubAccount>({
...
const rows: TableRow[] = [
	...
	{ label: 'Nama Sub Akun', field: 'name', align: 'left', isSort: { activeSort: 'asc' } },
	{ label: 'Group Perkiraan', field: 'account_group_name', align: 'left' },
	{ label: 'Saldo Normal', field: 'normal_balance', align: 'left' },
	{ label: '', field: 'action', align: 'right' }
]

function remove(row: SubAccount) {
	ask({ title: 'Hapus sub akun', message: `Hapus "${row.code} — ${row.name}"?`, type: 'danger', confirmText: 'Hapus' }, async () => {
		await api.delete(`/sub-account/${row.id}`)
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Sub Akun" subtitle="Master data sub akun">
			<template #actions>
				<Button size="sm" @click="router.push('/sub-account/tambah')"> <IconPlus class="h-4 w-4" /> Tambah Sub Akun </Button>
			</template>
		</PageHeader>
		...
		<Panel>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					<button
						v-if="row.field === 'code'"
						class="font-mono tnum text-primary-dark hover:underline"
						@click="router.push(`/sub-account/${column.id}`)"
					>
						{{ column.code }}
					</button>
					...
					<div v-else-if="row.field === 'action'" class="flex justify-end gap-1">
						<button class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill" @click="router.push(`/sub-account/${column.id}`)">
							<IconEye class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill"
							@click="router.push(`/sub-account/edit/${column.id}`)"
						>
							<IconPencil class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-danger-soft hover:text-danger"
							@click="remove(column as unknown as SubAccount)"
						>
