...
import { validateDepartment, type DepartmentForm } from '@/views/department/schema'

const props = defineProps<{ initial?: Partial<DepartmentForm>; isEdit?: boolean; submitLabel?: string; loading?: boolean }>()
const emit = defineEmits<{ submit: [DepartmentForm] }>()
...
function onSubmit() {
	const found = validateDepartment(form)
	errors.value = found ?? {}
	...
	<Panel>
		<form class="space-y-8" @submit.prevent="onSubmit">
			...
			<div class="flex gap-2 pt-2">
				...
				<Button variant="secondary" type="button" @click="router.push('/department')"> <IconX class="h-4 w-4" /> Batal </Button>
			</div>
		</form>
	</Panel>
</template>
