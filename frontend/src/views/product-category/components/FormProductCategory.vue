...
import { validateProductCategory, type ProductCategoryForm } from '@/views/product-category/schema'

const props = defineProps<{ initial?: Partial<ProductCategoryForm>; isEdit?: boolean; submitLabel?: string; loading?: boolean }>()
const emit = defineEmits<{ submit: [ProductCategoryForm] }>()
...
function onSubmit() {
	const found = validateProductCategory(form)
	errors.value = found ?? {}
	...
	<Panel>
		<form class="space-y-8" @submit.prevent="onSubmit">
			...
			<div class="flex gap-2 pt-2">
				...
				<Button variant="secondary" type="button" @click="router.push('/product-category')"> <IconX class="h-4 w-4" /> Batal </Button>
			</div>
		</form>
	</Panel>
</template>
