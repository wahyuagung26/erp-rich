...
import type { ProductCategory, TableRow } from '@/utils/types'

const router = useRouter()
const { ask } = useConfirm()
const toast = useToast()

const { columns, pagination, loading, fetchList, handleSort, pageTo, applyFilters } = useTableList<ProductCategory>({
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Kategori Produk" subtitle="Master data kategori produk">
			<template #actions>
				<Button size="sm" @click="router.push('/product-category/tambah')"> <IconPlus class="h-4 w-4" /> Tambah Kategori </Button>
			</template>
		</PageHeader>
		...
		<Panel>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					<button v-if="row.field === 'code'" class="text-primary-dark hover:underline" @click="router.push(`/product-category/${column.id}`)">
						{{ column.code }}
					</button>
					<div v-else-if="row.field === 'action'" class="flex justify-end gap-1">
						<button
							...
							@click="router.push(`/product-category/${column.id}`)"
						>
							<IconEye class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill"
							@click="router.push(`/product-category/edit/${column.id}`)"
						>
							<IconPencil class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-danger-soft hover:text-danger"
							@click="remove(column as unknown as ProductCategory)"
						>
