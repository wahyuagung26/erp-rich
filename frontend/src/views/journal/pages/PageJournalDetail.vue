...
import { journalStatusLabel, journalStatusTone } from '@/views/journal/schema'
import type { Journal, JournalStatus } from '@/utils/types'
...
async function confirmApprove() {
	...
	try {
		const res = await api.patch<{ data: Journal }>(`/journal/${value.id}/status`, { status: 'approved', date: approveDate.value })
...
function changeStatus(status: JournalStatus) {
	const value = journal.value
	if (!value || (status !== 'approved' && status !== 'rejected' && status !== 'submitted')) return
	const isResubmit = status === 'submitted'
	ask(
		{
			title: status === 'approved' ? 'Setujui jurnal' : status === 'rejected' ? 'Tolak jurnal' : 'Ajukan ulang jurnal',
			message: `${status === 'approved' ? 'Setujui' : status === 'rejected' ? 'Tolak' : 'Ajukan ulang'} jurnal "${value.number}"?`,
			type: status === 'rejected' ? 'danger' : 'info',
			confirmText: status === 'approved' ? 'Setujui' : status === 'rejected' ? 'Tolak' : 'Ajukan Ulang'
		},
		async () => {
			try {
				const res = await api.patch<{ data: Journal }>(`/journal/${value.id}/status`, { status })
				journal.value = res.data.data
				toast.success(isResubmit ? 'Jurnal diajukan ulang' : status === 'approved' ? 'Jurnal disetujui' : 'Jurnal ditolak')
			} catch (err) {
				if (axios.isAxiosError(err)) toast.error(err.response?.data?.message ?? 'Status jurnal tidak dapat diubah')
			}
		}
...
function remove() {
	...
	ask({ title: 'Hapus jurnal', message: `Hapus jurnal "${value.number}"?`, type: 'danger', confirmText: 'Hapus' }, async () => {
		try {
			await api.delete(`/journal/${value.id}`)
...
<template>
	<div class="max-w-5xl space-y-4 p-4">
		<PageHeader title="Detail Jurnal" subtitle="Jurnal Umum">
			<template #actions>
				<div class="flex flex-wrap items-center gap-2">
					<Badge v-if="journal" :tone="journalStatusTone[journal.status]" class="mr-1">{{ journalStatusLabel[journal.status] }}</Badge>
					<Button size="sm" variant="subtle" @click="router.push('/journal')"><IconArrowLeft class="h-4 w-4" /> Kembali</Button>
					<template v-if="journal?.status === 'submitted'">
						<Button size="sm" variant="secondary" @click="router.push(`/journal/edit/${journal.id}`)"><IconPencil class="h-4 w-4" /> Edit</Button>
			...
			<div class="space-y-8">
				...
				<section class="space-y-3">
					...
					<div class="overflow-x-auto">
						<table class="w-full min-w-[760px] text-m">
							<thead>
		...
		<Modal :open="approveOpen" title="Setujui Jurnal" size="sm" @close="approveOpen = false">
