...
const emit = defineEmits<{ submit: [{ date: string; description: string; lines: JournalLine[] }] }>()
...
const blank = (): JournalLine => ({ account_id: 0, debit: 0, credit: 0 })
const lines = ref<JournalLine[]>([blank(), blank()])
...
const filled = computed(() => lines.value.every((l) => l.account_id && (Number(l.debit) > 0 || Number(l.credit) > 0)))
...
function submit() {
	...
	emit('submit', {
		...form,
		lines: lines.value.map((l) => ({ account_id: Number(l.account_id), debit: Number(l.debit), credit: Number(l.credit) }))
	})
}
</script>
	...
	<Panel class="max-w-5xl">
		<form class="space-y-8" @submit.prevent="submit">
						...
						<tr v-for="(line, i) in lines" :key="i">
							<td class="py-1.5 pr-2">
								<Select
									...
									@update:model-value="(v) => (line.account_id = Number(v))"
								/>
							</td>
			...
			<div class="flex gap-2 pt-2">
				...
				<Button variant="secondary" type="button" @click="router.push('/journal')"> <IconX class="h-4 w-4" /> Batal </Button>
			</div>
		</form>
	</Panel>
</template>
