...
import { journalStatusLabel, journalStatusTone } from '@/views/journal-expense/schema'
import type { JournalExpense, JournalStatus } from '@/utils/types'
...
async function confirmApprove() {
	...
	try {
		const res = await api.patch<{ data: JournalExpense }>(`/journal-expense/${value.id}/status`, { status: 'approved', date: approveDate.value })
		journal.value = res.data.data
		approveOpen.value = false
		toast.success('Jurnal pengeluaran disetujui')
	} catch (err) {
...
function changeStatus(status: JournalStatus) {
	...
	ask(
		...
		async () => {
			try {
				const res = await api.patch<{ data: JournalExpense }>(`/journal-expense/${value.id}/status`, { status })
				journal.value = res.data.data
...
function remove() {
	...
	ask({ title: 'Hapus jurnal', message: `Hapus jurnal "${value.number}"?`, type: 'danger', confirmText: 'Hapus' }, async () => {
		try {
			await api.delete(`/journal-expense/${value.id}`)
			toast.success('Jurnal pengeluaran dihapus')
...
<template>
	<div class="max-w-5xl space-y-4 p-4">
		<PageHeader title="Detail Jurnal Pengeluaran" subtitle="Jurnal Pengeluaran">
			<template #actions>
				<div class="flex flex-wrap items-center gap-2">
					<Badge v-if="journal" :tone="journalStatusTone[journal.status]" class="mr-1">{{ journalStatusLabel[journal.status] }}</Badge>
					<Button size="sm" variant="subtle" @click="router.push('/journal-expense')"><IconArrowLeft class="h-4 w-4" /> Kembali</Button>
					<template v-if="journal?.status === 'submitted'">
						<Button size="sm" variant="secondary" @click="router.push(`/journal-expense/edit/${journal.id}`)"
						...
						<table class="w-full min-w-[720px] text-m">
							...
							<tbody>
								...
								<tr class="border-b border-hairline">
									...
									<td class="px-3 py-3 text-ink-subtle">Kas/bank keluar (otomatis)</td>
									<td class="px-3 py-3 text-right text-ink-subtle">–</td>
									<td class="px-3 py-3 text-right"><Amount :value="journal.cash_out" /></td>
								</tr>
							</tbody>
							<tfoot>
		...
		<Modal :open="approveOpen" title="Setujui Jurnal Pengeluaran" size="sm" @close="approveOpen = false">
