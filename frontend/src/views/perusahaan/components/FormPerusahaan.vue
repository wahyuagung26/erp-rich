...
import { validatePerusahaan, COMPANY_TYPES, type PerusahaanForm } from '@/views/perusahaan/schema'

const props = defineProps<{ initial?: Partial<PerusahaanForm>; submitLabel?: string; loading?: boolean }>()
const emit = defineEmits<{ submit: [PerusahaanForm] }>()
...
function onSubmit() {
	const found = validatePerusahaan(form)
	errors.value = found ?? {}
	...
	<Panel class="max-w-5xl">
		<form class="space-y-8" @submit.prevent="onSubmit">
			...
			<div class="flex gap-2 pt-2">
				...
				<Button variant="secondary" type="button" @click="router.push('/perusahaan')"> <IconX class="h-4 w-4" /> Batal </Button>
			</div>
		</form>
	</Panel>
</template>
