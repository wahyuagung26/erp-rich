...
import type { Cabang, TableRow } from '@/utils/types'

const router = useRouter()
const { ask } = useConfirm()
const toast = useToast()

const { columns, pagination, loading, fetchList, handleSort, pageTo, applyFilters } = useTableList<Cabang>({
	endpoint: '/cabang',
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Cabang" subtitle="Master data cabang perusahaan">
			<template #actions>
				<Button size="sm" @click="router.push('/cabang/tambah')"> <IconPlus class="h-4 w-4" /> Tambah Cabang </Button>
			</template>
		</PageHeader>
		...
		<Panel>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					<button v-if="row.field === 'code'" class="text-primary-dark hover:underline" @click="router.push(`/cabang/${column.id}`)">
						{{ column.code }}
					</button>
					...
					<div v-else-if="row.field === 'action'" class="flex justify-end gap-1">
						...
						<button
							...
							@click="remove(column as unknown as Cabang)"
						>
