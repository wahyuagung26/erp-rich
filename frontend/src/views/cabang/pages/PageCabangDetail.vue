		...
		<PageHeader title="Detail Cabang" subtitle="Master data cabang perusahaan">
			<template #actions>
				<div class="flex gap-2">
					<Button size="sm" variant="subtle" @click="router.push('/cabang')"> <IconArrowLeft class="h-4 w-4" /> Kembali </Button>
					<template v-if="cabang">
						<Button size="sm" @click="router.push(`/cabang/edit/${cabang.id}`)"> <IconPencil class="h-4 w-4" /> Edit </Button>
		...
		<Panel v-else>
			<section class="space-y-3">
				...
				<dl class="grid grid-cols-[140px_1fr] gap-x-4 gap-y-2.5 text-m">
					...
					<dd class="text-ink">{{ cabang.code }}</dd>
					<dt class="text-ink-muted">Nama Cabang</dt>
					<dd class="text-ink">{{ cabang.name }}</dd>
					<dt class="text-ink-muted">Alamat</dt>
					<dd class="whitespace-pre-line text-ink">{{ dash(cabang.address) }}</dd>
				</dl>
			</section>
		</Panel>
	</div>
