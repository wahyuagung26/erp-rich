...
import { journalStatusLabel, journalStatusTone, type JournalIncomeForm } from '@/views/journal-income/schema'
...
const toForm = (value: JournalIncome): JournalIncomeForm => ({
	number: value.number,
	date: value.date,
...
async function save(payload: JournalIncomeForm) {
	saving.value = true
	try {
		await api.put(`/journal-income/${route.params.id}`, payload)
		toast.success('Jurnal pemasukan diperbarui')
		router.push(`/journal-income/${route.params.id}`)
	} catch (err) {
		if (axios.isAxiosError(err)) toast.error(err.response?.data?.message ?? 'Jurnal pemasukan tidak dapat diperbarui')
		else throw err
	} finally {
		saving.value = false
	}
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Edit Jurnal Pemasukan" subtitle="Jurnal Pemasukan">
		...
		<Panel v-else-if="notFound || !journal" class="max-w-5xl">
			...
			<div class="mt-3 flex justify-center"><Button variant="secondary" @click="router.push('/journal-income')">Kembali</Button></div>
		</Panel>
		<Panel v-else-if="journal.status !== 'submitted'" class="max-w-5xl">
			...
			<div class="mt-3 flex justify-center">
				<Button variant="secondary" @click="router.push(`/journal-income/${journal.id}`)">Lihat Detail</Button>
			</div>
		</Panel>
		<FormJournalIncome v-else :initial-value="toForm(journal)" :loading="saving" submit-label="Simpan Perubahan" @submit="save" />
	</div>
</template>
