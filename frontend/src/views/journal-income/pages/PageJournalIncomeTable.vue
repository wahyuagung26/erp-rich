...
import { journalStatusLabel } from '@/views/journal-income/schema'
import type { JournalIncome, JournalStatus, TableRow } from '@/utils/types'

const router = useRouter()
const { columns, pagination, loading, fetchList, handleSort, pageTo, applyFilters } = useTableList<JournalIncome>({ endpoint: '/journal-income' })
...
async function confirmBulkApprove() {
	...
	for (const id of ids) {
		try {
			await api.patch(`/journal-income/${id}/status`, { status: 'approved' })
			ok++
		} catch {
...
const rows: TableRow[] = [
	...
	{ label: 'Nilai (Rp)', field: 'cash_in', align: 'right', isSort: { activeSort: 'asc' } },
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Jurnal Pemasukan" subtitle="Cash receipt">
			<template #actions>
				<Button size="sm" @click="router.push('/journal-income/tambah')"> <IconPlus class="h-4 w-4" /> Buat Jurnal </Button>
			</template>
		</PageHeader>
		...
		<Panel>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					...
					<Amount v-else-if="row.field === 'cash_in'" :value="column.cash_in as number" />
					<div v-else-if="row.field === 'action'" class="flex justify-end gap-1">
						<button
							...
							title="Lihat detail"
							@click="router.push(`/journal-income/${column.id}`)"
						>
						...
						<template v-if="(column.status as JournalStatus) === 'submitted'">
							<button
								class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill"
								title="Edit jurnal"
								@click="router.push(`/journal-income/edit/${column.id}`)"
							>
							...
							<button
								...
								title="Hapus jurnal"
								@click="remove(column as unknown as JournalIncome)"
							>
