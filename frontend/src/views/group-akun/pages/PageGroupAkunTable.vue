...
import type { GroupAkun, TableRow } from '@/utils/types'

const router = useRouter()
const { ask } = useConfirm()
const toast = useToast()

const { columns, pagination, loading, fetchList, handleSort, pageTo, applyFilters } = useTableList<GroupAkun>({
	endpoint: '/group-akun',
...
const rows: TableRow[] = [
	...
	{ label: 'Nama Group Akun', field: 'name', align: 'left', isSort: { activeSort: 'asc' } },
	{ label: 'Neraca / Laba Rugi', field: 'category', align: 'left' },
...
function remove(row: GroupAkun) {
	ask({ title: 'Hapus group akun', message: `Hapus "${row.code} — ${row.name}"?`, type: 'danger', confirmText: 'Hapus' }, async () => {
		await api.delete(`/group-akun/${row.id}`)
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Group Akun" subtitle="Master data group akun">
			<template #actions>
				<Button size="sm" @click="router.push('/group-akun/tambah')"> <IconPlus class="h-4 w-4" /> Tambah Group Akun </Button>
			</template>
		</PageHeader>
		...
		<Panel>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					<button v-if="row.field === 'code'" class="text-primary-dark hover:underline" @click="router.push(`/group-akun/${column.id}`)">
						{{ column.code }}
					</button>
					<span v-else-if="row.field === 'category'">{{ categoryLabel(column.category as string) }}</span>
					...
					<div v-else-if="row.field === 'action'" class="flex justify-end gap-1">
						<button class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill" @click="router.push(`/group-akun/${column.id}`)">
							<IconEye class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill"
							@click="router.push(`/group-akun/edit/${column.id}`)"
						>
							<IconPencil class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-danger-soft hover:text-danger"
							@click="remove(column as unknown as GroupAkun)"
						>
