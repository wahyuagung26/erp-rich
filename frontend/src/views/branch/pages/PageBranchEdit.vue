...
import FormBranch from '@/views/branch/components/FormBranch.vue'
import type { Branch } from '@/utils/types'
import type { BranchForm } from '@/views/branch/schema'
...
const formRef = ref<InstanceType<typeof FormBranch>>()
...
async function save(payload: BranchForm) {
	saving.value = true
	try {
		await api.put(`/branch/${route.params.id}`, payload)
	...
	<div class="space-y-4 p-4">
		...
		<Panel v-else-if="notFound || !branch">
			...
			<div class="mt-3 flex justify-center">
				<Button variant="secondary" @click="router.push('/branch')">Kembali</Button>
			</div>
		</Panel>
		<FormBranch v-else ref="formRef" :initial="branch" is-edit :loading="saving" submit-label="Simpan Perubahan" @submit="save" />
	</div>
</template>
