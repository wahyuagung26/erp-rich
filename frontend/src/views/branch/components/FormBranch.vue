...
import { validateBranch, type BranchForm } from '@/views/branch/schema'

const props = defineProps<{ initial?: Partial<BranchForm>; isEdit?: boolean; submitLabel?: string; loading?: boolean }>()
const emit = defineEmits<{ submit: [BranchForm] }>()
	...
	<Panel>
		<form class="space-y-8" @submit.prevent="onSubmit">
			...
			<div class="flex gap-2 pt-2">
				...
				<Button variant="secondary" type="button" @click="router.push('/branch')"> <IconX class="h-4 w-4" /> Batal </Button>
			</div>
		</form>
	</Panel>
</template>
