		...
		<PageHeader title="Detail Satuan" subtitle="Master data satuan produk">
			<template #actions>
				<div class="flex gap-2">
					<Button size="sm" variant="subtle" @click="router.push('/unit')"> <IconArrowLeft class="h-4 w-4" /> Kembali </Button>
					<template v-if="unit">
						<Button size="sm" @click="router.push(`/unit/edit/${unit.id}`)"> <IconPencil class="h-4 w-4" /> Edit </Button>
		...
		<Panel v-else>
			<section class="space-y-3">
				...
				<dl class="grid grid-cols-[140px_1fr] gap-x-4 gap-y-2.5 text-m">
					...
					<dd class="text-ink">{{ unit.code }}</dd>
					<dt class="text-ink-muted">Nama Satuan</dt>
					<dd class="text-ink">{{ unit.name }}</dd>
				</dl>
			</section>
		</Panel>
	</div>
