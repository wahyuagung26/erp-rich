...
import { validateUnit, type UnitForm } from '@/views/unit/schema'

const props = defineProps<{ initial?: Partial<UnitForm>; isEdit?: boolean; submitLabel?: string; loading?: boolean }>()
const emit = defineEmits<{ submit: [UnitForm] }>()
	...
	<Panel>
		<form class="space-y-8" @submit.prevent="onSubmit">
			...
			<div class="flex gap-2 pt-2">
				...
				<Button variant="secondary" type="button" @click="router.push('/unit')"> <IconX class="h-4 w-4" /> Batal </Button>
			</div>
		</form>
	</Panel>
</template>
