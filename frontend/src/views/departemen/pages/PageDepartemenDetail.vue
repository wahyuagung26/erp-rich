		...
		<PageHeader title="Detail Departemen" subtitle="Master data departemen">
			<template #actions>
				<div class="flex gap-2">
					<Button size="sm" variant="subtle" @click="router.push('/departemen')"> <IconArrowLeft class="h-4 w-4" /> Kembali </Button>
					<template v-if="departemen">
						<Button size="sm" @click="router.push(`/departemen/edit/${departemen.id}`)"> <IconPencil class="h-4 w-4" /> Edit </Button>
		...
		<Panel v-else>
			<section class="space-y-3">
				...
				<dl class="grid grid-cols-[140px_1fr] gap-x-4 gap-y-2.5 text-m">
					...
					<dd class="text-ink">{{ departemen.code }}</dd>
					<dt class="text-ink-muted">Nama Departemen</dt>
					<dd class="text-ink">{{ departemen.name }}</dd>
				</dl>
			</section>
		</Panel>
	</div>
