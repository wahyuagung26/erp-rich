...
import type { AkunPerkiraan, TableRow } from '@/utils/types'

const router = useRouter()
const { ask } = useConfirm()
const toast = useToast()

const { columns, pagination, loading, fetchList, handleSort, pageTo, applyFilters } = useTableList<AkunPerkiraan>({
	endpoint: '/akun-perkiraan',
...
const rows: TableRow[] = [
	...
	{ label: 'Sub Akun', field: 'sub_akun_name', align: 'left' },
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Akun Perkiraan" subtitle="Master data akun perkiraan">
			<template #actions>
				<Button size="sm" @click="router.push('/akun-perkiraan/tambah')"> <IconPlus class="h-4 w-4" /> Tambah Akun Perkiraan </Button>
			</template>
		</PageHeader>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					<button
						...
						@click="router.push(`/akun-perkiraan/${column.id}`)"
					>
						{{ column.code }}
					</button>
