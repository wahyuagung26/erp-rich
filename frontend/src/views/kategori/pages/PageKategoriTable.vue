...
import type { Kategori, TableRow } from '@/utils/types'

const router = useRouter()
const { ask } = useConfirm()
const toast = useToast()

const { columns, pagination, loading, fetchList, handleSort, pageTo, applyFilters } = useTableList<Kategori>({
	endpoint: '/kategori',
...
const rows: TableRow[] = [
	...
	{ label: 'Nama Kategori', field: 'name', align: 'left', isSort: { activeSort: 'asc' } },
	{ label: '', field: 'action', align: 'right' }
]

function remove(row: Kategori) {
	ask({ title: 'Hapus kategori', message: `Hapus "${row.code} — ${row.name}"?`, type: 'danger', confirmText: 'Hapus' }, async () => {
		await api.delete(`/kategori/${row.id}`)
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Kategori Produk" subtitle="Master data kategori produk">
			<template #actions>
				<Button size="sm" @click="router.push('/kategori/tambah')"> <IconPlus class="h-4 w-4" /> Tambah Kategori </Button>
			</template>
		</PageHeader>
		...
		<Panel>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					<button v-if="row.field === 'code'" class="text-primary-dark hover:underline" @click="router.push(`/kategori/${column.id}`)">
						{{ column.code }}
					</button>
					<div v-else-if="row.field === 'action'" class="flex justify-end gap-1">
						<button class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill" @click="router.push(`/kategori/${column.id}`)">
							<IconEye class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill"
							@click="router.push(`/kategori/edit/${column.id}`)"
						>
							<IconPencil class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-danger-soft hover:text-danger"
							@click="remove(column as unknown as Kategori)"
						>
