...
import type { Jurnal, TableRow } from '@/utils/types'

const router = useRouter()
const { columns, pagination, loading, handleSort, pageTo, applyFilters } = useTableList<Jurnal>({ endpoint: '/jurnal' })
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Jurnal Umum" subtitle="Journal entries">
			<template #actions>
				<Button size="sm" @click="router.push('/jurnal/tambah')"> <IconPlus class="h-4 w-4" /> Buat Jurnal </Button>
			</template>
		</PageHeader>
