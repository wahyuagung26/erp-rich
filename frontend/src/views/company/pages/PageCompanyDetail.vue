...
function remove() {
	...
	if (!c) return
	ask({ title: 'Hapus company', message: `Hapus "${c.code} — ${c.short_name}"?`, type: 'danger', confirmText: 'Hapus' }, async () => {
		await api.delete(`/company/${c.id}`)
...
<template>
	...
	<div class="max-w-5xl space-y-4 p-4">
		<PageHeader title="Detail Perusahaan" subtitle="Master data company / badan usaha">
			<template #actions>
				<div class="flex gap-2">
					<Button size="sm" variant="subtle" @click="router.push('/company')"> <IconArrowLeft class="h-4 w-4" /> Kembali </Button>
					<template v-if="company">
						<Button size="sm" @click="router.push(`/company/edit/${company.id}`)"> <IconPencil class="h-4 w-4" /> Edit </Button>
		...
		<Panel v-else-if="notFound || !company">
			...
			<div class="space-y-8">
				<section class="space-y-3">
					...
					<dl class="grid grid-cols-[180px_1fr] gap-x-4 gap-y-2.5 text-m">
						...
						<dd class="text-ink">{{ company.code }}</dd>
						<dt class="text-ink-muted">Nama Pendek</dt>
						<dd class="text-ink">{{ company.short_name }}</dd>
						<dt class="text-ink-muted">Nama Legal / PT</dt>
						<dd class="text-ink">{{ dash(company.legal_name) }}</dd>
						<dt class="text-ink-muted">NPWP</dt>
						<dd class="text-ink">{{ dash(company.npwp) }}</dd>
						<dt class="text-ink-muted">Logo</dt>
						<dd>
							<img v-if="company.logo_url" :src="company.logo_url" alt="Logo company" class="h-16 w-16 rounded-md object-contain" />
							<span v-else class="text-ink">–</span>
						</dd>
						<dt class="text-ink-muted">Alamat</dt>
						<dd class="whitespace-pre-line text-ink">{{ dash(company.address) }}</dd>
					</dl>
				</section>
				...
				<section class="space-y-3">
					...
					<dl class="grid grid-cols-[180px_1fr] items-center gap-x-4 gap-y-2.5 text-m">
						...
						<dd class="text-ink">{{ companyTypeLabel(company.company_type) }}</dd>
						<dt class="text-ink-muted">Pendataan Karyawan</dt>
						<dd class="text-ink">{{ company.hr_enabled ? 'Ya, aktif untuk modul HR' : 'Tidak' }}</dd>
						<dt class="text-ink-muted">Warna Header Laporan</dt>
						<dd class="flex items-center gap-2">
							<span class="inline-block h-5 w-8 rounded" :style="{ backgroundColor: company.report_header_color }" />
							<span class="font-mono tnum text-ink">{{ company.report_header_color }}</span>
						</dd>
					</dl>
				</section>
			</div>
