...
function remove(row: Customer) {
	ask({ title: 'Hapus customer', message: `Hapus "${row.code} — ${row.name}"?`, type: 'danger', confirmText: 'Hapus' }, async () => {
		await api.delete(`/customer/${row.id}`)
...
<template>
	<div class="space-y-4 p-4">
		<PageHeader title="Customer" subtitle="Master data pelanggan">
			<template #actions>
				<Button size="sm" @click="router.push('/customer/tambah')"> <IconPlus class="h-4 w-4" /> Tambah Customer </Button>
			</template>
		</PageHeader>
		...
		<Panel>
			...
			<Table :rows="rows" :columns="columns as unknown as Record<string, unknown>[]" :loading="loading" @handle-sort="handleSort">
				<template #table-content="{ row, column }">
					<button v-if="row.field === 'code'" class="text-primary-dark hover:underline" @click="router.push(`/customer/${column.id}`)">
						{{ column.code }}
					</button>
					...
					<div v-else-if="row.field === 'action'" class="flex justify-end gap-1">
						<button class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill" @click="router.push(`/customer/${column.id}`)">
							<IconEye class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-fill"
							@click="router.push(`/customer/edit/${column.id}`)"
						>
							<IconPencil class="h-4 w-4" />
						</button>
						<button
							class="grid h-7 w-7 place-items-center rounded-md text-ink-muted hover:bg-danger-soft hover:text-danger"
							@click="remove(column as unknown as Customer)"
						>
