...
**Lebar konten:** `DefaultLayout` sudah membungkus tiap halaman `mx-auto max-w-[1200px]`. Di dalamnya — halaman list: `<Panel>` full; halaman form dengan banyak field/grid 2 kolom (Supplier, Customer, Perusahaan): `<Panel class="max-w-5xl">` **rata kiri** (sejajar breadcrumb/judul), cap di Panel bukan di `<form>` dalam, jangan `mx-auto`. Form kecil (Merk, Kategori, Satuan, Cabang): `<Panel>` **full juga, sama seperti list** — jangan `max-w-xl`, sisa ruang kosong di sebelah kanan terasa aneh. Di dalamnya, cap lebar HANYA pada field kode/identifier singkat (`class="max-w-[200px]"`, sama seperti "Kode Supplier" yang disabled) — field teks bebas (nama, alamat, catatan) **tidak** diberi `max-w`, biarkan stretch mengikuti Panel, persis pola `FormSupplier.vue`.
